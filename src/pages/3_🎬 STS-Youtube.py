...
st.markdown(
    f"""
    <h1 style='text-align: center;'> Ask STS-BOT to summarize youtube video ! 😁</h1>
    """,
...
if not user_api_key:
    ...
    if video_url :
        ...
        if video_id != "":
            t = YouTubeTranscriptApi.get_transcript(video_id, languages=('en','fr','es','ar', 'zh-cn', 'hi', 'ar', 'bn', 'ru', 'pt', 'sw' ))
            finalString = ""
            for item in t:
                text = item['text']
                finalString += text + " "
